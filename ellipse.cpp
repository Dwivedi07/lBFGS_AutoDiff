...
// Fit ellipse: (x / a)^2 + (y / b)^2 = 1 rotated by theta
// Parameters: [a, b, theta]
// Cost function with autodiff using ceres::Jet
template<typename T>
T ellipseResidual(const T* params, const Eigen::Vector2d& pt) {
    T x0 = params[0];
    T y0 = params[1];
    T a  = params[2];
    T b  = params[3];
    ...
    T cos_t = ceres::cos(th);
    T sin_t = ceres::sin(th);

    T dx = pt[0] - x0;
    T dy = pt[1] - y0;

    T x_rot =  cos_t * dx + sin_t * dy;
    T y_rot = -sin_t * dx + cos_t * dy;

    T norm = x_rot / a;
    T nrm2 = norm * norm + (y_rot / b) * (y_rot / b);

    return nrm2 - T(1.0);  // residual = f(x)^2 = 0 when point lies on ellipse
}

class EllipseCost {
public:
    EllipseCost(const Eigen::MatrixXd& Y) : Y_(Y) {}

    Scalar operator()(const Vector& params, Vector& grad) {
        using JetT = ceres::Jet<Scalar, 5>; // 5 parameters
        Eigen::Matrix<JetT, Eigen::Dynamic, 1> jets(5);
        for (int i = 0; i < 5; ++i) {
            Eigen::Matrix<double, 5, 1> d = Eigen::Matrix<double, 5, 1>::Zero();
            d[i] = 1.0;
            jets[i] = JetT(params[i], d);
        }

        JetT cost = JetT(0.0);
        for (int i = 0; i < Y_.rows(); ++i) {
            JetT r = ellipseResidual(jets.data(), Y_.row(i));
            cost += r * r;
        }

        // Extract function value and gradient
        grad.resize(5);
        for (int i = 0; i < 5; ++i) {
            grad(i) = cost.v(i);
        }
...
int main() {
    // Load the data
    Eigen::MatrixXd Y(kYRows, kYCols);
    ...
    Vector params(5);
    params << 0.0, 0.0, 2.0, 1.0, 0.0;

    // Set bounds
    Vector lb(5), ub(5);
    lb << -1e2, -1e2, 0.01, 0.01, -M_PI;
    ub <<  1e2,  1e2,  10.0,  10.0,  M_PI;
    ...
    // Solve
    EllipseCost cost_fn(Y);
    Scalar fx;
    int niter = solver.minimize(cost_fn, params, fx, lb, ub);

    std::cout << "Solved in " << niter << " iterations\n";
    std::cout << "x0 = " << params[0] << ", y0 = " << params[1] << "\n";
    std::cout << "a  = " << params[2] << ", b  = " << params[3] << "\n";
    std::cout << "θ  = " << params[4] << "\n";
